...
class SyncUtils @Inject constructor(
    ...
    private val spotifyLikeMapper by lazy { SpotifyYouTubeMapper(database) }

    private suspend fun spotifySyncLikesEnabled(): Boolean =
        context.dataStore.data.first()[SpotifySyncLikesKey] ?: false

    fun likeSong(s: SongEntity) {
        syncScope.launch {
            // ── Spotify like sync (independent of YouTube login) ──
            if (runCatching { spotifySyncLikesEnabled() }.getOrDefault(false)) {
                runCatching {
                    val dbSong = database.song(s.id).firstOrNull()
                    val artist = dbSong?.artists?.firstOrNull()?.name ?: ""
                    val uri = spotifyLikeMapper.resolveToSpotifyUri(s.id, s.title, artist, s.duration)
                    if (uri != null) {
                        if (s.liked) Spotify.addToLibrary(listOf(uri))
                        else Spotify.removeFromLibrary(listOf(uri))
                    }
                }.onFailure { e ->
                    Timber.w(e, "Spotify like sync failed for ${s.id}")
                }
            }
            if (!isLoggedIn()) {
